data "archive_file" "kickoff_codescan_lambda_payload" {
  type       = "zip"
  source_dir = var.kickoff_codescan_lambda_src_dir
  excludes = [
    "venv",
    "_pycache_"
  ]
  output_path = "${var.kickoff_codescan_lambda_payload_dir}/payload.zip"
}

resource "aws_lambda_function" "kickoff_codescan" {
  filename         = data.archive_file.kickoff_codescan_lambda_payload.output_path
  function_name    = "kickoffCodeScan"
  description      = "Runs Event Team CodeBuild on CodeCommit push"
  role             = aws_iam_role.kickoff_lambda.arn
  handler          = "main.main"
  source_code_hash = data.archive_file.kickoff_codescan_lambda_payload.output_base64sha256
  runtime          = "python3.11"
  timeout          = 30
  publish          = "true"
  architectures    = ["arm64"]
...
resource "aws_lambda_permission" "codecommit_kickoff_codescan" {
  statement_id   = "AllowCodeCommitInvoke-kickoffCodeScan"
  action         = "lambda:InvokeFunction"
  function_name  = aws_lambda_function.kickoff_codescan.arn
