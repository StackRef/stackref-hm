...
var ACCESS_TOKEN_LENGTH = 16; // (apparent) length of an Auth0 access_token
...
var testClientOptions = function( domain, clientId ) {
    if ( typeof domain === "undefined" || ! domain.match( /\.auth0\.com$/ )  ) {
        throw new Error( "Expected AUTH0_DOMAIN or AUTH0_DOMAIN_PARAMETER environment variable to be set in .env file. See https://manage.auth0.com/#/applications" )
    }
...
var getClient = function() {
    if ( process.env.SR_AUTH0_DOMAIN && process.env.AUTH0_DOMAIN_PARAMETER ) {
        throw new Error( "Expected only one of AUTH0_DOMAIN and AUTH0_DOMAIN_PARAMETER environment variable to be set in .env file." )
    }
    if ( process.env.SR_AUTH0_CLIENT_ID && process.env.AUTH0_CLIENTID_PARAMETER ) {
    ...
    if ( process.env.AUTH0_DOMAIN_PARAMETER ) {
        ...
        domain = process.env.SR_AUTH0_DOMAIN
    }
    ...
    if ( process.env.AUTH0_CLIENTID_PARAMETER ) {
        ...
        clientId = process.env.SR_AUTH0_CLIENT_ID
    }
...
// extract user_id from the auth0 userInfo and return it for AWS principalId
