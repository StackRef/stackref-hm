...
from stackref.update_team_score_item import update_team_score_item
...
def process_post_method(event):
    ...
    if 'body' in event:
        ...
        if 'team_score_item' in payload_json:
            authorization = (
                'event_write' in grants,
                'judge' in get_participant_grants(user_uuid, get_event_by_team(payload_json['team_score_item']['team_uuid']))
            )
            if any(authorization):
                return update_team_score_item(payload_json)
            else:
