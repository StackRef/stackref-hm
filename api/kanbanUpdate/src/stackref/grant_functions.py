...
'''
    get_user_from_team_member
        Return the user_uuid that the team_member_uuid belongs to
'''
def get_user_from_team_member(team_member_uuid):
    if not team_member_uuid:
        return None

    sql_statement = ("""
    -- Get the User UUID of the team_member_uuid
    SELECT
        p.user_uuid
    FROM
        sr.team_member AS tm
    LEFT JOIN sr.participant AS p ON
        tm.participant_uuid = p.participant_uuid
    WHERE
        tm.team_member_uuid = %(team_member_uuid)s::UUID;
    """)
    log.debug(sql_statement)
    ...
    cached_data = retrieve_query_response('team_member', hashed_query)
    if cached_data:
        log.info(':: get_user_from_team_member: Using cached data')
        return cached_data
    else:
        log.info(':: Fetching data to cache')
        try:
            ...
            log.error(f">> get_user_from_team_member: {error}")
            return None

        if response and response[0]:
            user_uuid = str(response[0])
        else:
            user_uuid = None
...
'''
    get_team_member_from_user
        Using user_uuid and team_uuid, return the team_member_uuid if it exists
'''
def get_team_member_from_user(user_uuid, team_uuid):
    if not user_uuid or not team_uuid:
        return None

    sql_statement = ("""
    -- Get the Team Member UUID of the user_uuid and team_uuid
    SELECT
        tm.team_member_uuid
    FROM
        sr.team_member AS tm
    LEFT JOIN sr.participant AS p ON
        tm.participant_uuid = p.participant_uuid
    LEFT JOIN sr.user AS u ON
        p.user_uuid = u.user_uuid
    WHERE
        u.user_uuid = %(user_uuid)s::UUID AND
        tm.team_uuid = %(team_uuid)s::UUID;
    """)
    log.debug(sql_statement)
    ...
    cached_data = retrieve_query_response('team_member', hashed_query)
    if cached_data:
        log.info(':: get_team_member_from_user: Using cached data')
        return cached_data
    else:
        log.info(':: Fetching data to cache')
        try:
            ...
            log.error(f">> get_team_member_from_user: {error}")
            return None

        if response and response[0]:
            team_member_uuid = str(response[0])
        else:
            team_member_uuid = None

        cache_query_response('team_member', hashed_query, team_member_uuid)

        return team_member_uuid
