...
from stackref.archive_event import archive_event
from stackref.update_event import complete_event, update_event
    ...
    if 'body' in event:
        ...
        if (
            ...
            authorized = (
                ...
                'manage' in get_participant_grants(user_uuid, payload_json['event']['event_uuid'])
            )
            if not any(authorized):
                return return_error(401, "Not authorized for Event")
            elif payload_json['action'] == 'complete':
                return complete_event(payload_json)
            elif payload_json['action'] == 'update':
                return update_event(payload_json)
            elif payload_json['action'] == 'archive':
                return archive_event(payload_json)
            else:
