...
'''
    create_kickoff
        Create a CloudWatch rule to trigger on Event start
'''
def create_kickoff(event_uuid, payload_json):
    log.info(":: create_kickoff")
    ...
    try:
        schedule_group_res = scheduler_client.create_schedule_group(
        ...
        log.error(f'>> create_kickoff: {error}')
        raise error
    ...
    for schedule in schedules:
        ...
        if schedule['ts'] > datetime.now():
            try:
                ...
                schedule_res = scheduler_client.create_schedule(
                    Name=schedule_name,
