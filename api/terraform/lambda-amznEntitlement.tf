data "archive_file" "amzn_mkt_entitlement_lambda_payload" {
  type       = "zip"
  source_dir = var.amzn_mkt_entitlement_lambda_src_dir
  excludes = [
    "venv",
    "_pycache_"
  ]
  output_path = "${var.amzn_mkt_entitlement_lambda_payload_dir}/payload.zip"
}

resource "aws_lambda_function" "amzn_mkt_entitlement" {
  filename         = data.archive_file.amzn_mkt_entitlement_lambda_payload.output_path
  function_name    = "amznMktEntitlement"
  description      = "Reads/updates Amazon Marketplace Entitlements"
  role             = aws_iam_role.stackref_main_api.arn
  handler          = "main.main"
  source_code_hash = data.archive_file.amzn_mkt_entitlement_lambda_payload.output_base64sha256
  runtime          = "python3.11"
  timeout          = 60
  publish          = "true"
  architectures    = ["arm64"]
...
resource "aws_lambda_permission" "amzn_mkt_entitlement" {
  statement_id  = "AllowAPIGatewayInvoke-amznMktEntitlement"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.amzn_mkt_entitlement.arn
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.stackref_main.execution_arn}/*/*/amznMktEntitlement"
}
