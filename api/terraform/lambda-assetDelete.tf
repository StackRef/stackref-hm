data "archive_file" "asset_delete_lambda_payload" {
  type       = "zip"
  source_dir = var.asset_delete_lambda_src_dir
  excludes = [
    "venv",
    "_pycache_"
  ]
  output_path = "${var.asset_delete_lambda_payload_dir}/payload.zip"
}

resource "aws_lambda_function" "asset_delete" {
  filename         = data.archive_file.asset_delete_lambda_payload.output_path
  function_name    = "assetDelete"
  description      = "Deletes Assets in S3 and references in DB"
  role             = aws_iam_role.stackref_main_api.arn
  handler          = "main.main"
  source_code_hash = data.archive_file.asset_delete_lambda_payload.output_base64sha256
  runtime          = "python3.11"
  timeout          = 60
  publish          = "true"
  architectures    = ["arm64"]
...
resource "aws_lambda_permission" "asset_delete" {
  statement_id  = "AllowAPIGatewayInvoke-assetDelete"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.asset_delete.arn
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.stackref_main.execution_arn}/*/*/assetDelete"
}
