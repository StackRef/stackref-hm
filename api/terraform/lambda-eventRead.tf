data "archive_file" "event_read_lambda_payload" {
  type       = "zip"
  source_dir = var.event_read_lambda_src_dir
  excludes = [
    "venv",
    "_pycache_"
  ]
  output_path = "${var.event_read_lambda_payload_dir}/payload.zip"
}

resource "aws_lambda_function" "event_read" {
  filename         = data.archive_file.event_read_lambda_payload.output_path
  function_name    = "eventRead"
  description      = "Reads Events"
  role             = aws_iam_role.stackref_main_api.arn
  handler          = "main.main"
  source_code_hash = data.archive_file.event_read_lambda_payload.output_base64sha256
  runtime          = "python3.11"
  timeout          = 60
  publish          = "true"
  architectures    = ["arm64"]
...
resource "aws_lambda_permission" "event_read" {
  statement_id  = "AllowAPIGatewayInvoke-eventRead"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.event_read.arn
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.stackref_main.execution_arn}/*/*/eventRead"
}
