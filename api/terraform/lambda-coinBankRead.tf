data "archive_file" "coin_bank_read_lambda_payload" {
  type       = "zip"
  source_dir = var.coin_bank_read_lambda_src_dir
  excludes = [
    "venv",
    "_pycache_"
  ]
  output_path = "${var.coin_bank_read_lambda_payload_dir}/payload.zip"
}

resource "aws_lambda_function" "coin_bank_read" {
  filename         = data.archive_file.coin_bank_read_lambda_payload.output_path
  function_name    = "coinBankRead"
  description      = "Reads Coin Banks"
  role             = aws_iam_role.stackref_main_api.arn
  handler          = "main.main"
  source_code_hash = data.archive_file.coin_bank_read_lambda_payload.output_base64sha256
  runtime          = "python3.11"
  timeout          = 60
  publish          = "true"
  architectures    = ["arm64"]
...
resource "aws_lambda_permission" "coin_bank_read" {
  statement_id  = "AllowAPIGatewayInvoke-coinBankRead"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.coin_bank_read.arn
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.stackref_main.execution_arn}/*/*/coinBankRead"
}
