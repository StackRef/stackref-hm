...
def get_auth0_audience(event):
    ...
    if aud == settings.auth0_be_audience:
        return 'be'
    elif aud in settings.auth0_client_ids:
        return 'fe'
    else:
        return None
...
'''
    get_participant_from_user_event
        Return participant_uuid if user is a participant in the event
'''
def get_participant_from_user_event(user_uuid, event_uuid):
    if not user_uuid or not event_uuid:
        return None

    sql_statement = ("""
    -- Get Participant UUID if user is a participant in the event
    SELECT
        participant_uuid
    FROM
        sr.participant
    ...
    cached_data = retrieve_query_response('participant', hashed_query)
    if cached_data:
        log.info(':: get_participant_from_user_event: Using cached data')
        ...
        try:
            ...
            log.error(f">> get_participant_from_user_event: {error}")
            return None

        if response and response[0]:
            participant_uuid = str(response[0])
        else:
            participant_uuid = None

        # Don't cache these for long (5s)
        cache_query_response('participant', hashed_query, participant_uuid)

        return str(participant_uuid)

'''
    get_event_team_form_mode
        Return team_form_mode_name for Event
'''
def get_event_team_form_mode(event_uuid):
    sql_statement = ("""
    -- Get team_form_mode_name for Event
    SELECT
        etfm.event_team_form_mode_name
    FROM
        sr.event AS event
    LEFT JOIN sr.event_team_form_mode AS etfm ON
        etfm.event_team_form_mode_id = event.event_team_form_mode_id
    WHERE
    ...
    if cached_data:
        log.info(':: get_event_team_form_mode: Using cached data')
        return str(cached_data)
    else:
        log.info(':: Fetching data to cache')
        try:
            ...
            log.error(f">> get_event_team_form_mode: {error}")
            return None

        if response and response[0]:
            event_team_form_mode_name = str(response[0])
        else:
            event_team_form_mode_name = None

        cache_query_response('event', hashed_query, event_team_form_mode_name)

        return str(event_team_form_mode_name)
