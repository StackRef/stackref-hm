...
def process_post_method(event):
    ...
    if 'body' in event:
        ...
        authorization = (
            ...
            'manage' in get_participant_grants(user_uuid, get_event_by_team(payload_json['cloud_account_user']['entity_uuid'])),
            'manage' in get_team_member_grants(user_uuid, payload_json['cloud_account_user']['entity_uuid'])
        )
