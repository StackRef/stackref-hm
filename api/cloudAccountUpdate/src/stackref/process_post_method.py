...
from stackref.update_cloud_account import assign_cloud_account, unassign_cloud_account
...
def process_post_method(event):
    ...
    if 'body' in event:
        ...
        if (
            ...
            'cloud_account' in payload_json and
            payload_json['cloud_account']['entity_uuid']
        ):
            authorization = (
                'platform_write' in grants,
                'manage' in get_participant_grants(user_uuid, get_event_by_team(payload_json['cloud_account']['entity_uuid'])),
                'manage' in get_team_member_grants(user_uuid, payload_json['cloud_account']['entity_uuid'])
            )
            if any(authorization):
                if payload_json['action'] == 'assign':
                    return assign_cloud_account(payload_json)
                elif payload_json['action'] == 'unassign':
                    return unassign_cloud_account(payload_json)
                else:
                    return return_error(500, 'Unhandled action')
            else:
                return return_error(401, "Not authorized for Cloud Accounts")
        else:
