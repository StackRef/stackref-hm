...
from stackref.archive_judging_criterion import archive_judging_criterion
from stackref.update_judging_criterion import update_judging_criterion
...
def process_post_method(event):
    ...
    if 'body' in event:
        ...
        if (
            ...
            'criterion' in payload_json and
            payload_json['criterion']['judging_criterion_uuid']
        ):
            if 'organization_write' not in grants:
                return return_error(401, "Not authorized")
            elif payload_json['action'] == 'update':
                return update_judging_criterion(payload_json)
            elif payload_json['action'] == 'archive':
                return archive_judging_criterion(payload_json)
            else:
