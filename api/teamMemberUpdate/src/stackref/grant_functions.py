...
'''
    user_is_team_member
        Return True if user_uuid matches a team_member_uuid
'''
def user_is_team_member(user_uuid, team_member_uuid):
    if not user_uuid or not team_member_uuid:
        return False

    sql_statement = ("""
    -- Check if User matches Team Member
    SELECT
        1
    FROM
        sr.user AS u
    JOIN sr.participant AS p ON
        u.user_uuid = p.user_uuid
    JOIN sr.team_member AS tm ON
        p.participant_uuid = tm.participant_uuid
    WHERE
        p.user_uuid = %(user_uuid)s::UUID
        AND tm.team_member_uuid = %(team_member_uuid)s::UUID;
    """)
    log.debug(sql_statement)
    ...
    cached_data = retrieve_query_response('team_member', hashed_query)
    if cached_data:
        log.info(':: user_is_team_member: Using cached data')
        ...
        try:
            ...
            log.error(f">> user_is_team_member: {error}")
            return 0

        if response and response[0]:
            user_is_team_member = 1
        else:
            user_is_team_member = 0

        cache_query_response('team_member', hashed_query, user_is_team_member)

        return int(user_is_team_member)
