...
function EditTeamMemberRoles(props) {
  const { selectedTeamMember, team, getTeamMembers, ...other } = props;
  ...
  if (!team || !selectedTeamMember) return null;
            ...
            <Form noValidate onSubmit={handleSubmit} {...other}>
              <FormControl size='small' sx={{ m: 2, width: 150 }}>
                ...
                <Select
                  ...
                  input={<OutlinedInput />}
                >
                  {team.team_member_roles.map((role) => {
                    return (
                      <MenuItem
...
const TeamMemberListTable = (props) => {
  ...
  const handleAddNewTeamMember = async (event) => {
    if (newTeamMember.participant_uuid) {
      console.log(
        `:: handleAddNewTeamMember: ${newTeamMember.participant_uuid} to ${team.team_uuid}`,
      );
      try {
        setSubmitting(true);
        const payload = {};
